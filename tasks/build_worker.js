...
function wrapScriptWithModule(code, build) {
    return new Promise((resolve, reject) => {
        try {
            ...
            let wrappedCode = readFileSync(
                `${rootFolder}js/${moduleFileName}.js`,
                "utf8",
            );
...
async function modularizeCode(code, build) {
    const globalScope =
        build == "canvas_worker" || build == "lottie_worker"
            ? "self"
            : "window";
...
async function getCode() {
    try {
        let scriptsString = "";
        scriptsString += readFileSync(`${buildFolder}lottie.js`, {
            encoding: "utf8",
        });
        scriptsString += "\r\n";
...
async function buildVersion(version) {
    try {
        ...
        const modularizedCode = await modularizeCode(
            processedCode,
            version.build,
        );
...
function save(code, fileName) {
    return new Promise((resolve, reject) => {
        writeFile(`${buildFolder}${fileName}`, code, (err) => {
            if (err) {
                reject(err);
            } else {
                resolve(true);
